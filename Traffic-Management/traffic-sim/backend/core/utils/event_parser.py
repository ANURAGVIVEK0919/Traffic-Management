...
def parse_event_log(events):
	...
	for event in events or []:
		if not isinstance(event, dict):
			continue
		if event.get('eventType') in ('vehicle_added', 'vehicle_crossed', 'signal_phase', 'rl_decision') or \
		   event.get('event_type') in ('vehicle_added', 'vehicle_crossed', 'signal_phase', 'rl_decision'):
