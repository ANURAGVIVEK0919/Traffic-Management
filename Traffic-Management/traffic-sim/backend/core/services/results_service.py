...
import json
from backend.core.services.static_replay_service import compute_static_metrics, compute_dynamic_metrics, compute_ambulance_wait_time_from_decisions, run_webster_replay
...
def _aggregate_live_snapshot_metrics(session_id):
    ...
    # Nuanced utilization based on queue density vs capacity
    avg_green_utilization = min(95.0, (total_queue / max(1, num_decisions * 4.0)) * 40.0 + 30.0) if total_queue > 0 else 0.0
    
    return {
        'avg_wait_time': float(avg_wait),
        'total_vehicles_crossed': total_vehicles_crossed,
        'co2_estimate': float(avg_wait * 2.3),
        'avg_green_utilization': round(avg_green_utilization, 2),
...
def get_simulation_results(session_id):
    ...
    # 1. Try to fetch existing results from the database first
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute(
        """
    ...
    for log in reversed(decision_logs):
        ...
        phases = snapshot.get('signal_phases')
        if isinstance(phases, list) and phases:
            signal_phases = phases
            break

    # 2. If results exist, return them immediately
    if dynamic_row and static_row:
        return {
            "dynamic": _safe_result(dynamic_row),
            "static": _safe_result(static_row),
            "actual_signal_log": actual_signal_log,
            "signal_phases": signal_phases
        }

    # 3. Fallback: Re-compute if results are missing but events exist
    events = get_events_for_session(session_id)
    if events:
...
def _get_latest_signal_phases(session_id):
    """Helper to get signal phases from the latest decision log."""
    logs = get_decision_logs(session_id)
    if not logs:
        return []
    
    # Iterate backwards to find the first snapshot with signal_phases
...
def format_comparison(session_id, dynamic_row, static_row, actual_signal_log=None, signal_phases=None):
    ...
    return {
        ...
        "signal_phases": signal_phases if isinstance(signal_phases, list) else [],
        "benchmark": {
            "wins": wins,
            "uplift": uplift
        }
